...
- **govulncheck (hard-fail):** `go install golang.org/x/vuln/cmd/govulncheck@latest` then `govulncheck ./...`. Pinning note: a moving `@latest` can introduce surprise failures; acceptable for a security scanner (we *want* new-CVE signal), but the plan may pin a known-good version if flakiness appears.
