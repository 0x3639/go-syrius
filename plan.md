...
>
> **Dependency note (2026-08-22):** `znn-sdk-go` is pinned to **v0.5.0** (go.mod `go 1.26.0`, `toolchain go1.26.7`). v0.4.0 was a CodeRabbit security-hardening release whose breaking signature changes (`crypto.DeriveKey*` and `pow.GeneratePoW*` now return errors) touch no API go-syrius calls; behaviourally, `zenon.Zenon.Send`/`PrepareBlock` now run PoW under a 5-minute default `PowTimeout` and a difficulty budget, and leave the template untouched on failure (retry-safe with `TxService.clearPendingIf`). v0.5.0 only raises the Go floor to 1.26 (1.24/1.25 are EOL with reachable stdlib TLS/x509/net advisories).
>
> **Dependency note (2026-07-14, updated 2026-08-14):** `znn-sdk-go` was pinned to **v0.3.1**. v0.3.0 was the security-hardening release; its five breaking signature changes — `utils.GetTransactionBytes/GetTransactionHash/BigIntToBytes/BigIntToBytesSigned` and `pow.BenchmarkPoW` now returning errors — touch no API go-syrius calls. v0.3.1 fixes a v0.3.0 regression where `SubscriberApi` subscribe methods panicked (instead of erroring) on a client detached by `RpcClient.Stop()` — hit by `StartAutoReceive` racing a node disconnect (P1 in the PR #13 review; regression-tested in `app/node_service_autoreceive_race_test.go`). The tagged stable SDK does not expose the testnet governance extension previously consumed from a pseudo-version, so that narrow RPC/template adapter now lives under `internal/governance` and reuses the SDK's public transport and embedded-contract APIs.
>
> **WalletConnect note (2026-07-16):** WalletConnect v2 bridge support is implemented on `codex/walletconnect-bridges`. Both `0x3639/bridge-dapp` and `nom-bridge` use the same frozen `zenon:1` namespace (`znn_info`, `znn_sign`, `znn_send`), so one wallet implementation supports both. Pairing/session transport lives in the WebView, while Go exclusively validates, builds, PoWs, signs, and publishes. Mainnet writes remain disabled until the user explicitly enables them in Settings. Live pairing succeeded with both `https://bridge.0x3639.com` and the local `nom-bridge`; each displayed the expected `zenon:1:<active-address>` session. Follow-up security reviews hardened publication/result-delivery state, lock/session/account races, the single pending slot, canonical Wrap/Redeem funding, client retry, restored-session reconciliation, lifecycle test coverage, and abandoned error-hold cleanup. The 2026-07-17 audit (`docs/walletconnect-audit-2026-07-17.md`) is fully remediated: publication is durable/idempotent via a signed-block journal with hash-query/exact-rebroadcast reconciliation (`docs/superpowers/specs/2026-07-17-walletconnect-durable-publication.md`), request/proposal expiry is enforced, custom-token confirmations show exact base units and fail on unresolvable decimals, the mainnet opt-in is re-checked at broadcast, Verify identity is surfaced (scam peers blocked), node-URL disclosure is root-origin-only, and peer icons are never fetched. No real-funds transaction was submitted during pairing acceptance or review verification.
...
| Backend | Go 1.26+, `znn-sdk-go` v0.5.0, `go-zenon` | Already-built, tested SDK + native node |
