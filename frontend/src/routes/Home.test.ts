...
vi.mock('../../wailsjs/go/app/NodeService', () => ({ StartAutoReceive: vi.fn(), StopAutoReceive: vi.fn(), GetTransactions: vi.fn().mockResolvedValue([]), GetUnreceived: vi.fn().mockResolvedValue([]) }))
vi.mock('../../wailsjs/go/app/TxService', () => ({ PrepareSend: vi.fn(), ConfirmPublish: vi.fn(), CancelPending: vi.fn(), Receive: vi.fn() }))
...
vi.mock('../lib/stores/txs', () => ({ txs: { subscribe: (f: any) => { f([]); return () => {} } }, loadTxs: vi.fn() }))
vi.mock('../lib/stores/unreceived', () => ({ unreceived: { subscribe: (f: any) => { f([]); return () => {} } }, loadUnreceived: vi.fn() }))
...
describe('Home', () => {
  ...
  it('renders the recent transactions history section', () => {
    render(Home)
