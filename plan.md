...
>
> **Dependency note (2026-07-14, updated 2026-08-14):** `znn-sdk-go` is pinned to **v0.3.0** (security-hardening release; its five breaking signature changes — `utils.GetTransactionBytes/GetTransactionHash/BigIntToBytes/BigIntToBytesSigned` and `pow.BenchmarkPoW` now returning errors — touch no API go-syrius calls). The tagged stable SDK does not expose the testnet governance extension previously consumed from a pseudo-version, so that narrow RPC/template adapter now lives under `internal/governance` and reuses the SDK's public transport and embedded-contract APIs.
>
...
| Backend | Go 1.25.13+, `znn-sdk-go` v0.3.0, `go-zenon` | Already-built, tested SDK + native node |
